...
**TODO** kulon sectionbe a cbs es az inetes implementacio

- cbs overview -> external source
- cbs in inet
- the configuration
...
The :ned:`Ieee8021qCreditBasedShaper` module's :par:`idleSlope` parameter determines the
outgoing data rate of the shaper in bits per second. The shaper allows
...
set to zero. **TODO** emphasize that this is the most important? as this controls the committed information rate

Furthermore, the :par:`sendSlope` parameter 

Other parameters include the :par:`sendSlope`, which is the consumption rate of credits during transmission,
effectively the transmission rate of the interface; the 

Other parameters include the following:

- :par:`sendSlope`: The consumption rate of credits during transmission, effectively the transmission rate of the interface (default: idleSlope - bitrate) **TODO** why is this the tx rate of the interface?
...
we aim for the traffic to be only modified significantly by the credit-based shaper, avoiding 
any unintended traffic shaping effects elsewhere in the
network. To achieve this, we set up two traffic source applications in the
client, creating two separate data streams whose throughput varies
sinusoidally with average values of ~37.7 Mbps and ~16.7 Mbps,
respectively. Given these values, on average, the network links are
not operating at their full capacity. Subsequently, we configure the traffic
shaper to cap the data rates of these streams at ~42 Mbps and ~21 Mbps,
respectively. As a result, the average incoming data rate is lower than the
outgoing limit. Below are the details of the traffic configuration:
...
We enable egress traffic shaping in the network interface, which
adds the time-aware shaper module to the MAC layer. In the time-aware shaper, we define two
...
Let's take a look at how the traffic changes in the various network nodes. First, we
...
The data rate of the incoming traffic is on average less than the shaper's limit,
but it periodically goes above it. When this happens, the shaper
