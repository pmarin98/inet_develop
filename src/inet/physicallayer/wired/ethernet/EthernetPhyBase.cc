...
const double EthernetPhyBase::SPEED_OF_LIGHT_IN_CABLE = 200000000.0;

const EthernetPhyBase::EtherDescr EthernetPhyBase::nullEtherDescr = {
    0.0,
    0.0,
    0,
    B(0),
...
const EthernetPhyBase::EtherDescr EthernetPhyBase::etherDescrs[NUM_OF_ETHERDESCRS] = {
    {
        ETHERNET_TXRATE,
        0.5 / ETHERNET_TXRATE,
        0,
...
EthernetPhyBase::EthernetPhyBase()
{
...
{
    PhyProtocolBase::initialize(stage);
...
void EthernetPhyBase::initializeFlags()
{
...
void EthernetPhyBase::initializeStatistics()
{
...
void EthernetPhyBase::handleStartOperation(LifecycleOperation *operation)
{
...
void EthernetPhyBase::handleCrashOperation(LifecycleOperation *operation)
{
//    clearQueue();
    connected = false;
    networkInterface->setCarrier(false);
...
void EthernetPhyBase::receiveSignal(cComponent *source, simsignal_t signalID, cObject *obj, cObject *details)
{
    Enter_Method("%s", cComponent::getSignalName(signalID));

    PhyProtocolBase::receiveSignal(source, signalID, obj, details);
...
void EthernetPhyBase::processConnectDisconnect()
{
    if (!connected) {
...
void EthernetPhyBase::encapsulate(Packet *frame)
{
...
void EthernetPhyBase::decapsulate(Packet *packet)
{
...
bool EthernetPhyBase::verifyCrcAndLength(Packet *packet)
{
    EV_STATICCONTEXT;
...
void EthernetPhyBase::refreshConnection()
{
...
void EthernetPhyBase::printParameters()
{
    // Dump parameters
...
void EthernetPhyBase::refreshDisplay() const
{
    PhyProtocolBase::refreshDisplay();
...
void EthernetPhyBase::changeTransmissionState(MacTransmitState newState)
{
    transmitState = newState;
    emit(transmissionStateChangedSignal, newState);
}

void EthernetPhyBase::changeReceptionState(MacReceiveState newState)
{
    receiveState = newState;
    emit(receptionStateChangedSignal, newState);
}

void EthernetPhyBase::addPaddingAndSetFcs(Packet *packet, B requiredMinBytes) const
{
...
void EthernetPhyBase::cutEthernetSignalEnd(EthernetSignalBase *signal, simtime_t duration)
{
...
void EthernetPhyBase::txFinished()
{
